...
public class LocationManager extends CordovaPlugin implements BeaconConsumer {
    ...
    private void initBluetoothListener() {
        ...
        } catch (final BleNotAvailableException e) {
        ...
        broadcastReceiver = new BroadcastReceiver() {
            ...
            private String getStateDescription(final int state) {
                switch (state) {
                    case BluetoothAdapter.ERROR:
                        return "ERROR";
                    case BluetoothAdapter.STATE_OFF:
    ...
    private void checkIfDomSignaldDelegateReady() {
        ...
        final String warning = "WARNING did not receive delegate ready callback from DOM after " + CDV_LOCATION_MANAGER_DOM_DELEGATE_TIMEOUT + " seconds!";
        debugWarn(warning);
                    ...
                    public void run() {
                        try {
                            ...
                            if (eventType.equals("didDetermineStateForRegion")) {
                                final String stateName = nameOfRegionState(state);
            ...
            public void didRangeBeaconsInRegion(final Collection<Beacon> iBeacons, final Region region) {
                ...
                threadPoolExecutor.execute(new Runnable() {
                    public void run() {
                        ...
                        try {
                            final JSONObject data = new JSONObject();
                            final JSONArray beaconData = new JSONArray();
                            for (final Beacon beacon : iBeacons) {
    ...
    private void createManagerCallbacks(final CallbackContext callbackContext) {
        beaconServiceNotifier = new IBeaconServiceNotifier() {
            ...
            private void sendFailEvent(final String eventType, final Region region, final Exception exception, final CallbackContext callbackContext) {
                try {
                    final JSONObject data = new JSONObject();
     ...
     */
    private void onDomDelegateReady(final CallbackContext callbackContext) {
        ...
        _handleCallSafely(callbackContext, new ILocationManagerCommand() {
            ...
            public PluginResult run() {
                try {
                    ...
                    final boolean available = bluetoothAdapter != null && bluetoothAdapter.isEnabled();
    ...
    private void disableDebugNotifications(final CallbackContext callbackContext) {
    ...
    private void enableDebugNotifications(final CallbackContext callbackContext) {
        _handleCallSafely(callbackContext, new ILocationManagerCommand() {
    ...
    private void disableDebugLogs(final CallbackContext callbackContext) {
    ...
    private void enableDebugLogs(final CallbackContext callbackContext) {
        _handleCallSafely(callbackContext, new ILocationManagerCommand() {
    ...
    private void appendToDeviceLog(final String message, final CallbackContext callbackContext) {
        _handleCallSafely(callbackContext, new ILocationManagerCommand() {
    ...
    private void stopMonitoringForRegion(final JSONObject arguments, final CallbackContext callbackContext) {
        ...
        _handleCallSafely(callbackContext, new ILocationManagerCommand() {
            ...
            public PluginResult run() {
                ...
                try {
                    final Region region = parseRegion(arguments);
    ...
    private void stopRangingBeaconsInRegion(final JSONObject arguments, final CallbackContext callbackContext) {
        _handleCallSafely(callbackContext, new ILocationManagerCommand() {
    ...
    private void getAuthorizationStatus(final CallbackContext callbackContext) {
        _handleCallSafely(callbackContext, new ILocationManagerCommand() {
            ...
            public PluginResult run() {
                ...
                try {
                    ...
                    //Check the Bluetooth service is running
                    final String authStatus = iBeaconManager.checkAvailability()
    ...
    private void requestWhenInUseAuthorization(final CallbackContext callbackContext) {
        _handleCallSafely(callbackContext, new ILocationManagerCommand() {
    ...
    private void requestAlwaysAuthorization(final CallbackContext callbackContext) {
        _handleCallSafely(callbackContext, new ILocationManagerCommand() {
    ...
    private void getMonitoredRegions(final CallbackContext callbackContext) {
        ...
        _handleCallSafely(callbackContext, new ILocationManagerCommand() {
            ...
            public PluginResult run() {
                try {
                    final Collection<Region> regions = iBeaconManager.getMonitoredRegions();
    ...
    private void getRangedRegions(final CallbackContext callbackContext) {
        ...
        _handleCallSafely(callbackContext, new ILocationManagerCommand() {
            ...
            public PluginResult run() {
                try {
                    final Collection<Region> regions = iBeaconManager.getRangedRegions();
                    final JSONArray regionArray = new JSONArray();
                    for (final Region region : regions) {
                        regionArray.put(mapOfRegion(region));
                    }
    ...
    private void requestStateForRegion(final JSONObject arguments, final CallbackContext callbackContext) {
        ...
        _handleCallSafely(callbackContext, new ILocationManagerCommand() {
            ...
            public PluginResult run() {
                ...
                final PluginResult result = new PluginResult(PluginResult.Status.ERROR, "Manual request for monitoring update is not supported on Android");
                result.setKeepCallback(true);
                return result;
    ...
    private void isRangingAvailable(final CallbackContext callbackContext) {
        ...
        _handleCallSafely(callbackContext, new ILocationManagerCommand() {
            ...
            public PluginResult run() {
                try {
                    ...
                    //Check the Bluetooth service is running
                    final boolean available = iBeaconManager.checkAvailability();
                    return new PluginResult(PluginResult.Status.OK, available);
    ...
    private void registerDelegateCallbackId(final JSONObject arguments, final CallbackContext callbackContext) {
    ...
    private void isMonitoringAvailableForClass(final JSONObject arguments, final CallbackContext callbackContext) {
        _handleCallSafely(callbackContext, new ILocationManagerCommand() {
            ...
            public PluginResult run() {
                ...
                final PluginResult result = new PluginResult(PluginResult.Status.OK, isValid);
                result.setKeepCallback(true);
                return result;
    ...
    private void isAdvertisingAvailable(final CallbackContext callbackContext) {
    ...
    private void isAdvertising(final CallbackContext callbackContext) {
    ...
    private void startAdvertising(final JSONObject arguments, final CallbackContext callbackContext) throws JSONException {
        debugLog("Advertisement start START BEACON ");
        debugLog(arguments.toString(4));
        final String identifier = arguments.getString("identifier");
    ...
    private void stopAdvertising(final CallbackContext callbackContext) {
        ...
        _handleCallSafely(callbackContext, new ILocationManagerCommand() {
            ...
            public PluginResult run() {
                ...
                //not supported on Android
                final PluginResult result = new PluginResult(PluginResult.Status.OK, "iBeacon Advertising stopped.");
                result.setKeepCallback(true);
                return result;
    ...
    private Region parseRegion(final JSONObject json) throws JSONException, InvalidKeyException, UnsupportedOperationException {
    ...
    private Region parseCircularRegion(final JSONObject json) throws JSONException, InvalidKeyException, UnsupportedOperationException {
    ...
    private Region parseBeaconRegion(final JSONObject json) throws JSONException, UnsupportedOperationException {

        final String identifier = json.getString("identifier");

        //For Android, uuid can be null when scanning for all beacons (I think)
        final String uuid = json.has("uuid") && !json.isNull("uuid") ? json.getString("uuid") : null;
        final String major = json.has("major") && !json.isNull("major") ? json.getString("major") : null;
        final String minor = json.has("minor") && !json.isNull("minor") ? json.getString("minor") : null;

        if (major == null && minor != null)
            throw new UnsupportedOperationException("Unsupported combination of 'major' and 'minor' parameters.");

        final Identifier id1 = uuid != null ? Identifier.parse(uuid) : null;
        final Identifier id2 = major != null ? Identifier.parse(major) : null;
        final Identifier id3 = minor != null ? Identifier.parse(minor) : null;
        return new Region(identifier, id1, id2, id3);
    }


    private String nameOfRegionState(final int state) {
        switch (state) {
    ...
    private JSONObject mapOfRegion(final Region region) throws JSONException {
    ...
    private JSONObject mapOfBeaconRegion(final Region region) throws JSONException {
        final JSONObject dict = new JSONObject();
    ...
    private JSONObject mapOfBeacon(final Beacon region) throws JSONException {
        final JSONObject dict = new JSONObject();
    ...
    private String nameOfProximity(final double accuracy) {
    ...
    private boolean hasBlueToothPermission() {
        ...
        final int access = context.checkCallingOrSelfPermission(Manifest.permission.BLUETOOTH);
        final int adminAccess = context.checkCallingOrSelfPermission(Manifest.permission.BLUETOOTH_ADMIN);
    ...
    private void _handleCallSafely(final CallbackContext callbackContext, final ILocationManagerCommand task, final boolean runInBackground) {
        if (runInBackground) {
    ...
    private void _handleExceptionOfCommand(final CallbackContext callbackContext, final Exception exception) {
    ...
    private void _sendResultOfCommand(final CallbackContext callbackContext, final PluginResult pluginResult) {
    ...
    private void debugWarn(final String message) {
        if (debugEnabled) {
            Log.w(TAG, message);
        }
    }
